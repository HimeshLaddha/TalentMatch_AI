...
export default function AdminDashboard() {
  ...
  // Tab or Accordion details selection: 'xai' | 'prompts' (we can show all at once in separate visual cards)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.matches && response.matches.length > 0) {
        setSelectedCandidate(response.matches[0]); // default to first match
      }
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : "Failed to process matching pipeline. Please verify the backend is active.";
      setError(errMsg);
      setMatchData(null);
    } finally {
      setIsLoading(false);
  ...
  // Helper to standardise and display score formats (float/percent)
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Analyze unstructured job descriptions and match them with vector-indexed candidates.
          </p>
        </div>
      ...
      {/* Main Grid: JD Input Form on Left/Top, Results Leaderboard on Right/Bottom */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl p-6 shadow-xl backdrop-blur-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="position-title" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">
                Position Title
              </label>
              <input
                id="position-title"
            ...
            <div>
              <label htmlFor="position-domain" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">
            ...
            <div>
              <label htmlFor="job-description" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          {error && (
            <div className="bg-rose-950/20 border border-rose-500/30 rounded-2xl p-4 flex items-start gap-3 text-rose-300 text-sm animate-fade-in">
              <svg className="w-5 h-5 mt-0.5 text-rose-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="bg-slate-955 border border-slate-800/80 rounded-2xl p-6 shadow-xl backdrop-blur-md flex-1">
            ...
            {!isLoading && !matchData && (
              <div className="h-64 border border-dashed border-slate-800 rounded-xl flex flex-col items-center justify-center text-center p-6 bg-slate-900/10">
                ...
                <h3 className="font-semibold text-slate-300 text-sm">No Evaluation Data</h3>
            ...
            {isLoading && (
              <div className="space-y-3">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="h-14 bg-slate-900/40 border border-slate-800/50 rounded-xl animate-pulse flex items-center justify-between px-4">
            ...
            {/* Results Table */}
            {!isLoading && matchData && matchData.matches && matchData.matches.length > 0 && (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800 text-xs font-semibold uppercase tracking-wider text-slate-500 bg-slate-950/30">
                      ...
                      <th className="py-3 px-4 text-center hidden md:table-cell">Role Fit (40%)</th>
                      <th className="py-3 px-4 text-center hidden md:table-cell">Trajectory (30%)</th>
                      <th className="py-3 px-4 text-center hidden lg:table-cell">Signals (20%)</th>
                      <th className="py-3 px-4 text-center hidden lg:table-cell">Domain (10%)</th>
                    </tr>
                  </thead>
                    ...
                    {matchData.matches.map((match, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-4 px-4 text-center">
                            <span className={`inline-block font-bold text-sm px-2.5 py-1 rounded-full border ${getScoreColorClass(finalPct)}`}>
                              {finalPct}%
                            </span>
                          </td>
      ...
      {/* Accordion / Detailed Layer section below */}
          ...
          <div>
            ...
            {selectedCandidate && (
              <p className="text-slate-400 text-xs mt-0.5">
                Inspect AI breakdown analysis for candidate <span className="font-mono text-indigo-400 font-bold">{selectedCandidate.name}</span> ({selectedCandidate.candidate_id})
              </p>
            )}
          </div>
          ...
          {selectedCandidate && (
            <div className="flex bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-md transition-colors ${activeDetailTab === "all" ? "bg-indigo-600 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}
              >
                All Insights
              </button>
              <button
                onClick={() => setActiveDetailTab("fit")}
                className={`px-3 py-1 rounded-md transition-colors ${activeDetailTab === "fit" ? "bg-indigo-600 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}
              >
                🟢 Alignment
              </button>
              <button
                onClick={() => setActiveDetailTab("gaps")}
                className={`px-3 py-1 rounded-md transition-colors ${activeDetailTab === "gaps" ? "bg-indigo-600 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}
              >
                🔴 Gaps
              </button>
              <button
                onClick={() => setActiveDetailTab("prompts")}
                className={`px-3 py-1 rounded-md transition-colors ${activeDetailTab === "prompts" ? "bg-indigo-600 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}
              >
                🎤 Prompts
              </button>
            </div>
        ...
        {!selectedCandidate ? (
          <div className="h-32 border border-dashed border-slate-800 rounded-xl flex items-center justify-center text-center p-4 bg-slate-900/10">
            <p className="text-slate-500 text-xs max-w-sm">
              Please analyze a job description and select a candidate from the table above to view deep fit alignment metrics and screening guides.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            {/* Good Fit alignment indicators */}
            {(activeDetailTab === "all" || activeDetailTab === "fit") && (
              <div className="flex flex-col bg-emerald-950/20 border border-emerald-800/30 rounded-xl p-5 shadow-lg relative overflow-hidden group">
                ...
                <p className="text-emerald-300/90 text-sm leading-relaxed whitespace-pre-line flex-1 bg-slate-900/30 border border-emerald-950/30 rounded-lg p-3">
                  {selectedCandidate.strongest_alignment || selectedCandidate.strongly_aligned || "No explicit alignment indicators generated."}
                </p>
              </div>
            )}

            {/* Competency Gaps indicators */}
              ...
              <div className="flex flex-col bg-indigo-950/20 border border-indigo-800/30 rounded-xl p-5 shadow-lg relative overflow-hidden group md:col-span-1">
                ...
                <div className="space-y-2.5 flex-1 flex flex-col justify-between">
                  <div className="space-y-2 bg-slate-900/40 rounded-lg p-3 border border-indigo-950/30 overflow-y-auto max-h-52 font-mono text-xs text-indigo-300/90 leading-relaxed scrollbar-thin">
                    {selectedCandidate.tailored_interview_prompts && selectedCandidate.tailored_interview_prompts.length > 0 ? (
                      selectedCandidate.tailored_interview_prompts.map((promptText, i) => (
                        <div key={i} className="p-2 border-b border-indigo-900/20 last:border-b-0 flex gap-2">
                          <span className="text-indigo-500 font-bold shrink-0">{i+1}.</span>
                          <span className="select-all">{promptText}</span>
                        </div>
                      ))
                    ) : (
                      <span className="text-slate-500 italic">No tailored screening prompts provided.</span>
                    )}
                  </div>
                </div>
              </div>
